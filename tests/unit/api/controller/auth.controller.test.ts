...
const mockRequest = (body: any = {}, headers: any = {}, user: any = undefined): any => ({
  body,
  headers,
  user,
});
...
describe('AuthController', () => {
  ...
  describe('login', () => {
    it('should return tokens on successful login', async () => {
      ...
      await controller.login(req, res, mockNext); expect(mockAuthService.login).toHaveBeenCalledWith('user', 'pass');
  ...
  describe('refresh', () => {
    it('should return new tokens on successful refresh', async () => {
      const refreshData = { refreshToken: 'old_refresh_token' };
      const req = mockRequest(refreshData);
      const res = mockResponse();
    ...
    it('should call next with error on refresh failure', async () => {
      const refreshData = { refreshToken: 'invalid_token' };
      const req = mockRequest(refreshData);
      const res = mockResponse();
  ...
  describe('confirmSignUp', () => {
    it('should return 200 on successful confirmation', async () => {
      const confirmData = { username: 'user', confirmationCode: '123456' };
      const req = mockRequest(confirmData);
      const res = mockResponse();
    ...
    it('should call next with error on confirmation failure', async () => {
      const confirmData = { username: 'user', confirmationCode: 'wrongcode' };
      const req = mockRequest(confirmData);
      const res = mockResponse();
  ...
  describe('signOut', () => {
    it('should return 204 on successful signOut', async () => {
      const req = mockRequest({}, { authorization: 'Bearer valid_token' }, { accessToken: 'valid_token' });
      const res = mockResponse();
    ...
    it('should call next with AuthenticationError if auth header is missing', async () => {
      ...
      expect(mockNext).toHaveBeenCalledWith(expect.any(AuthenticationError));
      expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ message: 'Access token missing from authenticated request.' }));
    });

    it('should call next with error on signOut failure', async () => {
      const req = mockRequest({}, { authorization: 'Bearer invalid_token' }, { accessToken: 'invalid_token' });
      const res = mockResponse();
  ...
  describe('forgotPassword', () => {
    it('should return 200 with message on success', async () => {
      const forgotData = { username: 'user' };
      const req = mockRequest(forgotData);
      const res = mockResponse();
  ...
  describe('changePassword', () => {
    it('should return 200 on successful password change', async () => {
      ...
      const req = mockRequest(changeData, { authorization: 'Bearer valid_token' });
      const res = mockResponse();
    ...
    it('should call next with AuthenticationError if auth header is missing', async () => {
      ...
      const req = mockRequest(changeData);
      const res = mockResponse();
