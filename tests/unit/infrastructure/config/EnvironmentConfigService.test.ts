...
describe('EnvironmentConfigService', () => {
    ...
    const originalEnv = { ...process.env }; // Deep copy to avoid interference
    ...
    describe('constructor validation', () => {
        it('should throw an error if a required environment variable is missing', () => {
            ...
            expect(() => new EnvironmentConfigService()).toThrow(
                '[ConfigService] Missing or empty required environment variables: PORT'
            );
        });
        ...
        it('should throw an error if a required environment variable is an empty string', () => {
            ...
            expect(() => new EnvironmentConfigService()).toThrow(
                '[ConfigService] Missing or empty required environment variables: LOG_LEVEL'
            );
        });
    });
    ...
    describe('getNumber', () => {
        ...
        it('should throw an error when environment variable is invalid and no default provided', () => {
            ...
            expect(() => configService.getNumber('INVALID_NUMBER')).toThrow(
                'Configuration error: Environment variable "INVALID_NUMBER" is not a valid number ("not-a-number").'
            );
        });
    ...
    describe('getBoolean', () => {
        ...
        it('should throw an error when environment variable is invalid and no default provided', () => {
            process.env.INVALID_BOOL = 'not-a-boolean';
            configService = new EnvironmentConfigService();
            expect(() => configService.getBoolean('INVALID_BOOL')).toThrow(
                `Configuration error: Environment variable "INVALID_BOOL" is not a valid boolean ("not-a-boolean"). Expected 'true', 'false', '1', or '0'.`
            );
        });
    ...
    describe('getAllConfig', () => {
        it('should return non-sensitive environment variables and mask sensitive ones', () => {
            ...
            configService = new EnvironmentConfigService(); // Re-instantiate to pick up new vars
    ...
    describe('reloadConfig', () => {
        ...
        it('should re-validate required keys after reload', () => {
            ...
            delete process.env.PORT;
            expect(() => configService.reloadConfig()).toThrow(
                '[ConfigService] Missing or empty required environment variables after reload: PORT'
            );
        });
    });
});
